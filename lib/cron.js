  ...
  self.init = function( init_si, init_opts, cb ){
    ...
    si.add({role:self.name,cmd:'stopjob'},function(args,cb){
      ...
      if ( _.isUndefined(cronJobs[id]) ){
        si.log('cannot stop job, job not found', id, cronJobs)
        cb()// here I must report error
      }
      else{
        var job = cronJobs[id]
        job.stop();
    ...
    si.add({role:self.name,cmd:'startjob'},function(args,cb){
      ...
      if ( _.isUndefined(cronJobs[id]) ){
        si.log('cannot start job, job not found', id, cronJobs)
        cb()// here I must report error
      }
      else{
        var job = cronJobs[id]
    ...
    si.add({role:self.name,cmd:'closejob'},function(args,cb){
      ...
      if ( _.isUndefined(cronJobs[id]) ){
        si.log('cannot close job, job not found', id, cronJobs)
        cb()// here I must report error
      }
      else{
        var job = cronJobs[id]
